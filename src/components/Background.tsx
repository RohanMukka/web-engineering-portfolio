...
const Background = () => {
  ...
  const springMouseXInverse = useTransform(springMouseX, (val: number) => val * -0.5);
  const springMouseYInverse = useTransform(springMouseY, (val: number) => val * -0.5);
  const springMouseXDeep = useTransform(springMouseX, (val: number) => val * 1.5);
  const springMouseYDeep = useTransform(springMouseY, (val: number) => val * -1.2);
  const yParallaxHalf = useTransform(yParallax, (val: number) => val * 0.5);
        ...
        <motion.div
          ...
          style={{ 
            background: isDark 
              ...
              : 'radial-gradient(circle, #002366 0%, transparent 70%)',
            x: springMouseXDeep,
            y: springMouseYDeep,
          }}
        />
